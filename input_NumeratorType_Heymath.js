function input_NumeratorType_Heymath(mov,notxt,NegativeTxt,myDDName,fractionNumber) {
		 var mainMov;
		 var i=0;
		 var thisTextNum=0;
		 var thisSubTextNum=0;
		 var totalCount=0;
		 var riteCount=0;
		 var rongCount=0;
		 var playedCount=0;
		 var totalPairs=0;
		 var txtBoxNameWithOutPair=new Array  ;
		 var txtBoxNameNumerator=new Array  ;
		 var txtBoxNameDenominator=new Array  ;
		 var txtBoxName=new Array  ;
		 var myAnsTick1=new Array  ;
		 var myAnsTick2=new Array  ;
		 var myAns0=new Array  ;
		 var myAns1=new Array  ;
		 var myAns2=new Array  ;
		 var txtbxno;
		 var myDropDown;
		 var fractionInputStatus;//this is to chk if there is fraction input then true and some fraction and normal input then false....
		...
		//
		 ...
		 this.input_NumeratorType_Heymath = function(mov,notxt,NegativeTxt,myDDName,fractionNumber) {
		 ...
		 function init() {
			...
			for (i=0; i<=Number(txtBoxName.length-1); i++) {
				...
				mainMov[txtBoxName[i]].addEventListener("focusin",focusIn_fn);
				mainMov[txtBoxName[i]].addEventListener("focusout",focusOut_fn);
			}
			mainMov.tabChildren=false;
			mainMov.msg_clp.gotoAndStop("none");
			mainMov.chk_clp.check_btn.visible=true;
			mainMov.chk_clp.check_btn.addEventListener("click",check_fn);
		}
			...
			if (playedCount==0) {
				...
				for (i=1; i<=Number(txtBoxNameWithOutPair.length-1); i++) {
					...
					if (fractionInputStatus==false) {
						if (mainMov[txtBoxNameWithOutPair[i]].text==myAns0[i]) {
							...
							mainMov[txtBoxNameWithOutPair[i]].removeEventListener("focusin",focusIn_fn);
							mainMov[txtBoxNameWithOutPair[i]].removeEventListener("focusout",focusOut_fn);
				...
				for (i=1; i<=Number(txtBoxNameNumerator.length-1); i++) {
					...
					if (mainMov[txtBoxNameNumerator[i]].text==myAns1[i]&&mainMov[txtBoxNameDenominator[i]].text==myAns2[i]) {
						...
						mainMov[txtBoxNameNumerator[i]].removeEventListener("focusin",focusIn_fn);
						mainMov[txtBoxNameNumerator[i]].removeEventListener("focusout",focusOut_fn);
						mainMov[txtBoxNameDenominator[i]].removeEventListener("focusin",focusIn_fn);
						mainMov[txtBoxNameDenominator[i]].removeEventListener("focusout",focusOut_fn);
						riteCount++;
...
stopSnapSound();
 ...
 mainMov.root.playMultiSound();
				mainMov.chk_clp.check_btn.removeEventListener("click",check_fn);
				mainMov.chk_clp.check_btn.visible=false;
			}
		}//function to remove the front zeros..
		 function removeFrontZero(MyTxt) {
 ...
 this.input_NumeratorType_Heymath(mov,notxt,NegativeTxt,myDDName,fractionNumber); 
}
